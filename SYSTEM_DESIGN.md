...
know whether it is sold. At show-creation time `instantiate_show_seats()`
...
passes rows through `effective_status()`, which treats `HELD` past
...
**The sweeper is the visibility mechanism.** It flips expired rows and
broadcasts, so other people's screens stop showing a seat as grey. One `asyncio`
task running two indexed `UPDATE`s on a ten-second sleep, not a queued job: an
idle ARQ worker's blocking poll would burn Upstash's 500,000-command monthly
free tier in about three days, silently. Redis stays for email and the
Socket.IO adapter, where it earns its place.

**Holds run on two clocks.** Abandonment gets the full five minutes; an explicit
Back shortens the lease to fifteen seconds rather than deleting it, keeping the
owner so `extend_hold()` can restore the full TTL if they return. No new
mechanism — `effective_status()` already frees a lapsed lease.
...
Three details are load-bearing. `ORDER BY ss.id` prevents the deadlock when two
customers request the same pair in opposite orders; Postgres breaks a deadlock
by killing a transaction, turning a clean conflict into a 500. `FOR UPDATE OF
ss` locks only `ShowSeat`, not the joined `Seat`, which would serialise
unrelated shows in the same venue. And the query both locks and reads, so the
lock is held for two round trips rather than four — with four, seven of twenty
contenders timed out into 500s. Holds are all-or-nothing: a partial hold is
worse UX than a clean rejection and leaks seats when the cart is abandoned.
...
`advance_waitlist(session, show_seat_id)` is the **only** implementation of "a
seat became free, find the next customer". Cancellation calls it; offer expiry
calls the same function. Two copies drift on precisely the clauses that matter,
and the bug then appears only on the rarer path.
...
Expiry marks the entry `EXPIRED` and calls `advance_waitlist()` again rather than
